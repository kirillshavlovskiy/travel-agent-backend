 ...
 */
export function enhanceDailyPlansWithPreferences(
  dailyPlans: any[],
  preferences: {
 ...
 */
function generateAccessibilityTransportation(accessibility: string[]): string[] {
  const suggestions: string[] = [];
  
  if (accessibility.includes('wheelchair_accessible')) {
    suggestions.push(
      'Use accessible metro stations marked with wheelchair symbols',
      'Consider pre-booking wheelchair accessible taxis',
      'Paris Bus routes 24, 73, and 85 are fully accessible'
    );
  }
  
  if (accessibility.includes('limited_mobility')) {
    suggestions.push(
      'Plan shorter walking routes between activities',
      'Allow extra time for transportation between venues',
      'Consider Paris City Pass for priority access at attractions'
    );
  }
 ...
 */
function generateAccessibilityNotes(accessibility: string[], activities: any[]): string[] {
  const notes: string[] = [];
  
  if (accessibility.includes('wheelchair_accessible')) {
    notes.push(
      'All recommended activities have wheelchair access',
      'Request accessibility maps at museum entrances',
      'Use accessible entrances (often marked separately)'
    );
  }
  
  if (accessibility.includes('limited_mobility')) {
    notes.push(
      'Rest areas are available at all venues',
      'Consider renting mobility equipment if needed',
      'Elevator access available at all multi-level venues'
    );
  }
 ...
 */
function enhanceDiningBreakSuggestion(
  suggestion: string,
  dietaryRestrictions: string[]
): string {
  if (!suggestion) {
  ...
  return suggestion.replace(
    /at (recommended |local )?restaurant/i,
    `at ${dietaryRestrictions.length === 1 ? dietaryText + '-friendly' : 'accommodating'} restaurant with ${dietaryText} options`
  );
}
 ...
 */
function generateDietaryNotes(dietaryRestrictions: string[], activities: any[]): string[] {
  const notes: string[] = [];
  
  const restrictionsText = dietaryRestrictions.join(' and ');
  
  notes.push(
    `Paris offers many ${restrictionsText} dining options`,
    `Look for "Menu ${restrictionsText.charAt(0).toUpperCase() + restrictionsText.slice(1)}" signs at restaurants`,
    'Inform servers about dietary needs when ordering'
  );
  ...
  if (foodActivities.length > 0) {
    notes.push(
      `Notify the guide about ${restrictionsText} requirements before food activities`,
      'Consider contacting food tour operators in advance to confirm accommodations'
    );
  }
