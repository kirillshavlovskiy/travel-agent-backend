...
export class DeepSeekService {
  ...
  async searchRestaurants(params: RestaurantSearchParams): Promise<DeepSeekResponse> {
    try {
      if (!this.apiKey) {
...
Return ONLY a valid JSON object without any explanatory text, following this structure:
{
  ...
  private constructSearchQuery(params: RestaurantSearchParams): string {
    const {
      city,
      cuisine,
      priceRange,
  ...
  async getRestaurantDetails(restaurantName: string, city: string): Promise<Restaurant | null> {
    try {
  ...
  async createFoodItinerary(params: FoodItineraryParams): Promise<FoodItinerary | { error: string }> {
    try {
      if (!this.apiKey) {
        throw new Error('DeepSeek API key is not configured');
      }
...
Return ONLY a valid JSON object without any explanatory text, following this structure:
{
  ...
  private validateFoodItinerary(data: any): data is FoodItinerary {
  ...
  private constructFoodItineraryQuery(params: FoodItineraryParams): string {
    const {
      city,
      days,
      cuisine,
