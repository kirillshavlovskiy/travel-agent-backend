...
export function getPreferredTimeSlot(category: string): 'morning' | 'afternoon' | 'evening' {
...
export function getTypicalDuration(category: string): number {
  const categoryInfo = ACTIVITY_CATEGORIES.find(c => c.name === normalizeCategory(category));
  return categoryInfo?.typicalDuration || 120;
}

// Helper function to determine category based on keywords
export function determineCategoryFromDescription(description: string): string {
