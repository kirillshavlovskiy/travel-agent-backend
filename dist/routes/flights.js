    ...
    try {
        ...
        if (flightNumber && carrierCode) {
            filteredOffers = flightOffers.filter((offer) => {
                ...
                const outboundMatch = offer.itineraries[0]?.segments?.some((seg) => seg.carrierCode === carrierCode && seg.number === flightNumber);
                // Check inbound segments if any
                const inboundMatch = offer.itineraries[1]?.segments?.some((seg) => seg.carrierCode === carrierCode && seg.number === flightNumber);
...
router.post('/confirm-price', async (req, res) => {
    ...
    try {
        ...
        const flightOffer = flightOffers.find((offer) => offer.id === flightOfferId);
        if (!flightOffer) {
...
router.post('/booking-url', async (req, res) => {
    try {
        const { flightId, airline = '', origin, destination, departureDate, returnDate, cabinClass = 'ECONOMY', travelers = 1 } = req.body;
...
router.post('/seatmap', async (req, res) => {
    try {
        ...
        logger.info('Successfully retrieved seat map', {
            ...
            totalSeats: seatMap.segments.reduce((acc, seg) => acc + (seg.seats?.length || 0), 0)
        });
