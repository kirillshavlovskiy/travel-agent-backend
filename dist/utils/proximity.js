 ...
 */
export function assignNeighborhood(activity) {
    ...
    for (const [name, info] of Object.entries(PARIS_NEIGHBORHOODS)) {
        const distance = calculateDistance(coordinates.lat, coordinates.lng, info.lat, info.lng);
 ...
 */
export function buildDistanceMatrix(activities) {
 ...
 */
export function clusterActivitiesByProximity(activities, days, withAccessibilityNeeds = false, includeExtraOptions = false) {
 ...
 */
function optimizeRouteForDay(activities) {
    ...
    const optimizedRoute = [activities[0]];
 ...
 */
export function createProximityBasedSchedule(activities, days, accessibilityNeeds = [], pacePreference = 'moderate') {
 ...
 */
function createFastPacedSchedule(activities, days, withAccessibilityNeeds) {
    ...
    const uniqueActivities = Array.from(new Map(sortedActivities.map(item => [item.name, item])).values());
    ...
    for (let dayNumber = 1; dayNumber <= days; dayNumber++) {
        ...
        timeSlots.forEach((timeSlot, slotIndex) => {
            ...
            neighborhoodGroups.forEach((activitiesInNeighborhood, neighborhood) => {
                ...
                if (activitiesInNeighborhood.length === 0)
                    return;
 ...
 */
function assignTimeSlots(activities, dayNumber, withAccessibilityNeeds) {
